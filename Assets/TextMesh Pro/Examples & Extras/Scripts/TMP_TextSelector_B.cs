...
namespace TMPro.Examples
{
    public class TMP_TextSelector_B : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IPointerClickHandler, IPointerUpHandler
    {
        ...
        void RestoreCachedVertexAttributes(int index)
        {
            ...
            // Restore UV0S
            // UVS0
            Vector2[] src_uv0s = m_cachedMeshInfoVertexData[materialIndex].uvs0.Select(v => (Vector2)v).ToArray();
            Vector2[] dst_uv0s = m_TextMeshPro.textInfo.meshInfo[materialIndex].uvs0.Select(v => (Vector2)v).ToArray();
            ...
            // UVS0
            src_uv0s = m_cachedMeshInfoVertexData[materialIndex].uvs0.Select(v => (Vector2)v).ToArray();
            dst_uv0s = m_TextMeshPro.textInfo.meshInfo[materialIndex].uvs0.Select(v => (Vector2)v).ToArray();
